...
const FreeBots = observer(() => {
    ...
    const getBotDescription = (botName: string): string => {
        const descriptions: { [key: string]: string } = {
            'STARTER BOT':
                'Official starter bot for FreezyTradingHub. Optimized for beginners with standard risk management.',
            'POVERTY KILLER':
            ...
            'FREEZYTRADINGHUB AUTOMATED RISE FALL':
            ...
            'FREEZYTRADINGHUB RISE/FALL V4':
                'Latest version of the FreezyTradingHub Rise/Fall bot. Enhanced with improved entry signals and advanced recovery management for maximum stability.',
                        };
        ...
        const loadBots = async () => {
            ...
            try {
                ...
                const skeletonBots: BotData[] = manifest.map(item => {
                    const botName = (item.name || item.file.replace('.xml', '')).replace(/[_-]/g, ' ').replace('CAXYNEXUS-AIRISE FALLV4', 'FREEZYTRADINGHUB RISE/FALL V4');
                    const isPremiumPlus = botName.includes('FREEZYTRADINGHUB RISE/FALL V4');
                    return {
                        name: botName,
                        description: getBotDescription(botName),
                        difficulty: 'Intermediate',
